...
export function LoginPage({ onLogin, onNavigate }: LoginPageProps) {
  ...
  const handleDemoSignIn = async () => {
    setEmail('guest@serenitywellness.com');
    setPassword('demo1234');
    setError(null);
    setLoading(true);
    const res = await onLogin('guest@serenitywellness.com', 'demo1234');
    setLoading(false);
    if (res.success) {
      onNavigate('/my-bookings');
    }
    ...
    <div className="min-h-screen pt-28 pb-20 px-4 flex items-center justify-center bg-[#08080b]">
      <div className="w-full max-w-md p-8 sm:p-10 rounded-3xl bg-zinc-950 border border-white/10 shadow-2xl relative">
        <div className="text-center mb-8">
          ...
          <h1 className="font-display font-bold text-2xl text-white tracking-tight">Client Portal</h1>
          <p className="text-xs text-zinc-400 mt-1 font-mono">Access your voice-booked wellness itinerary</p>
        </div>
        ...
        <div className="mt-4">
          <button
            ...
            className="w-full py-2.5 rounded-xl bg-white/5 hover:bg-white/10 text-zinc-300 font-mono text-xs border border-white/10 transition-colors"
          >
